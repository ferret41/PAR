...
jgraph -P strong-omp.jgr > $PROG-$size-$np_NMIN-$np_NMAX-$N-strong-${HOST}.ps
usuario=`whoami`
fecha=`date`
sed -i -e "s/UUU/$usuario/g" $PROG-$size-$np_NMIN-$np_NMAX-$N-strong-${HOST}.ps
sed -i -e "s/FFF/$fecha/g" $PROG-$size-$np_NMIN-$np_NMAX-$N-strong-${HOST}.ps
